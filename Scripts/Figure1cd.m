...
colors = paperColors(3:-1:1).';
[fig_lines,h] = R.plotSpikeRaster(1500,4000,regions=[70,32,12],colors=colors(1:end));
legend('off')
...
title(gca,'')
do_save && saveFig(fig_raster,fullfile(file_root,'Figures/Papier_figure/Good/Raster/Fig1c_raster'),'png',res=700,pause=1);

% save axes and lines as svg
delete(h)
%yline(ax_lines,cumsum(R.nNeurons(regs([3,2])))+0.5,Color=myColors(1),LineWidth=1.7)
do_save && saveFig(fig_lines,fullfile(file_root,'Figures/Papier_figure/Good/Fig1c_axes'),'svg');
...
start = 1870; stop = 2040;

% save raster as png
[fig_raster,axs] = makeFigure('raster','',[2,1],TileSpacig='none');
R.plotSpikeRaster(start,stop,states='all',regions=70,colors=paperColors(3),ax=axs(1));
set(axs,'XTick',[],'YTick',[],'YColor','none'); legend(axs(1),'off'); set(axs(1),'XColor','none'); set(axs(2),'Color','none')
do_save && saveFig(fig_raster,fullfile(file_root,'Figures/Papier_figure/Good/Raster/Fig1d_raster'),'png',res=700,pause=1);
...
[~,h] = R.plotFiringRates(start,stop,aval_window,states='all',regions=70,smooth=smooth,mode="ratio",ax=axs(2));
set(h,'Color',paperColors(3))
PlotIntervals(slow_avals.nr,'legend','off')
...
do_save && saveFig(fig_aval,fullfile(file_root,'Figures/Papier_figure/Good/Fig1d_axes'),'svg');