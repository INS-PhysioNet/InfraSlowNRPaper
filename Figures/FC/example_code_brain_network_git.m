...
[gx,gy,idx] = grid_communities(s); % BCT function
...
figure, boxplot(enorm,s); title('Norm entropy per system')

%% visualize edge time series : same as my code dFC

[~,idx] = sort(ci); dffidx = find(diff(ci(idx)));
figure, imagesc(ets(:,idx)',[0,1]); hold on;
